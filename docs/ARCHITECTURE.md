...
export type EnhanceRequest = {
  ...
  requestId: string;
  /** Which quality tier to run — see §3.3 */
  tier: EnhanceTier;
};

export type EnhanceTier = 'standard' | 'deep';
...
### 3.3 مستويان للجودة — لا نموذج واحد

نموذج Remini نفسه ملكية خاصة وغير متاح (`DISCUSSION.md §10.1`). البديل المعتمد
مستويان، يُختاران عبر حقل `tier` في الطلب:

| المستوى | `tier` | النموذج | التكلفة/صورة |
|---|---|---|---|
| ترميم عادي | `standard` | CodeFormer | $0.0066 |
| ترميم عميق | `deep` | Nano Banana Pro | $0.05 |

**قواعد:**

- المستوى العادي هو الافتراضي. المستوى العميق **لا يُستدعى تلقائياً أبداً** —
  فارق التكلفة بينهما **7.5 ضعفاً**، واستدعاؤه بالخطأ يقلب حسبة الفاتورة.
- المستوى العميق يُحتسب في الحدود اليومية بوزن أعلى، أو يُفرَد له حدّ خاص.
- `model` في النتيجة يسجّل النموذج الذي عمل فعلاً، لا المستوى المطلوب —
  لتُنسَب التكلفة بدقة (`§3.1`).
- المستوى العميق هو **المنتج المدفوع** متى فُعِّل الدفع؛ تُبنى البنية بحيث لا
  يتطلب ذلك إعادة كتابة.

⚠️ لا يُعتمد أي نموذج نهائياً قبل الاختبار المقارَن في `DISCUSSION.md §10.5`.
